<?php  
        ...
        if (isset($_GET["deleteMasConfirm"] )) {
                echo
                    "<script> 
                        Swal.fire({
                            icon: 'warning',
                            ...
                            if (result.isConfirmed) {
                                location = 'form.php?deleteMasAlert=req&lec_id={$_GET["lec_id"]}&mast_id={$_GET["mast_id"]}'
                            }else{
        ...
        if (isset($_GET["deleteMasAlert"])) {

            // คำสั่ง sql ในการลบข้อมูล ตาราง tbl_products โดยจะลบข้อมูลสินค้า Tr_id ที่ส่งมา
            $sql = "DELETE FROM master1 WHERE mast_id={$_GET["mast_id"]}";
